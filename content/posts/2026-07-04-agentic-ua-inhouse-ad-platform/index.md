...
> **声明**：本文为独立行业观察，基于公开招股书与公开报道整理，不代表任何雇主或客户观点，不构成投资建议；本工作室不承接游戏（含手游 / 休闲 / 中重度）行业相关咨询，行业分析仅作为方法论示例。
...
2. 从公开信息和一线大客户反馈的使用情况看，Navos 和 Tec-Creative 的实际使用率并不高，多数大客户与代投的核心交互仍停留在开户和预算充值层面；
3. 出海买量的大客户，要么自建（SHEIN / Temu 等），要么用 AppsFlyer / Singular / Segwise 这类专业工具，不会把核心投放决策交给代投的 SaaS。
...
【归因侧数据】AppsFlyer / Adjust / Singular / SKAN
   ↓
【广告主 inhouse 数据】应用 SDK / 用户行为 / LTV / 变现数据
```
...
| **大客户结构性流失** | 头部买家一旦决定自建就不回头（如 SHEIN / Temu 这类头部品牌），代投只剩中腰部——而中腰部的钱又少又难赚 |
...
这不是新观点。头部出海公司几乎无一例外都在自建。**问题是中腰部的公司怎么办**。
...
- ROI 预估模型：结合应用内变现曲线、获客转化漏斗、跨平台协同效应做预测
...
**❌ 坑 5：抄头部大厂的架构**
头部买家内部的投放平台通常是几百人团队做了几年的产物。中小团队照搬会死得很惨。中腰部公司需要的是**能力分层的自建路径**，不是终态架构。
...
- **重点：Agent 编排层**：跨媒体、跨账户、跨业务线的策略调度，这是真正的护城河
...
**展博增长实验室**做的就是这件事：**中腰部出海买家的 inhouse 增长平台咨询 + 交付**。
...
服务过的场景：出海应用增长平台 L2→L4、AB 实验平台建设、Ad Agent 架构设计、多平台归因体系重构。
